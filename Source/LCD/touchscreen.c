...
uint32_t LCD_TS_Read(PT_T * position) {
	uint32_t x, y;
	uint32_t b;
	result_type res_msg_dest;
	request_type req_msg_src;
	osStatus_t result;
	...
	if (b>0) {
		...
		// Read X Position
									...
									osDelay(TS_DELAY);
		req_msg_src.channelNum=LCD_TS_YU_CHANNEL;
		req_msg_src.request = &result_queue;
		osMessageQueuePut(request_queue,&req_msg_src, NULL, osWaitForever);
		FPTB->PSOR = MASK(DBG_TSXY);
		//result = osSemaphoreAcquire(TRIGGER_sem, osWaitForever);
		//if(result == osOK){
		//}
		result = osMessageQueueGet(result_queue,&res_msg_dest, NULL, osWaitForever);
		if(result == osOK && res_msg_dest.channelNum==LCD_TS_YU_CHANNEL){
		x = res_msg_dest.result;
		}
		FPTB->PCOR = MASK(DBG_TSXY);		
		// Read Y Position
											// Configure inputs to ADC
									LCD_TS_XL_PORT->PCR[LCD_TS_XL_BIT] &= ~PORT_PCR_MUX_MASK;
									...
									// Wait for the inputs to settle
									osDelay(TS_DELAY);
		req_msg_src.channelNum=LCD_TS_XL_CHANNEL;
		req_msg_src.request = &result_queue;
		osMessageQueuePut(request_queue,&req_msg_src, NULL, osWaitForever);
		//result = osSemaphoreAcquire(TRIGGER_sem, osWaitForever);
		//if(result == osOK){
		...
		//}
		result = osMessageQueueGet(result_queue,&res_msg_dest, NULL, osWaitForever);
		if(result == osOK && res_msg_dest.channelNum==LCD_TS_XL_CHANNEL){
		y = res_msg_dest.result;
		}	
