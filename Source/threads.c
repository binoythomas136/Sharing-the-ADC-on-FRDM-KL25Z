...
osMessageQueueId_t request_queue,result_queue;
osSemaphoreId_t Semaphore_print_complete,Semaphore_print,Semaphore_start;
...
void Create_OS_Objects(void) {
	...
	request_queue = osMessageQueueNew(2, sizeof(request_type), NULL);
	result_queue = osMessageQueueNew(2, sizeof(result_type), NULL);
	Semaphore_start = osSemaphoreNew(1, 0, NULL);
	Semaphore_print = osSemaphoreNew(1, 0, NULL);
	Semaphore_print_complete = osSemaphoreNew(1, 0, NULL);
}
